...
# %% [markdown]
# Now, I need to ensure the sentences are in the right format for the classifier, and create a new dataframe in which each row is a single sentence
...
cross_dressed_sentences = sentence_df[sentence_df['character_id'].isin(cross_dressed_characters)]
print(cross_dressed_sentences.shape)

sentence_df = sentence_df[~sentence_df['character_id'].isin(cross_dressed_characters)]
print(sentence_df.shape)
...
df = sentence_df[['id','character_id','is_male', 'sentence']]


#cross_dressed_df = cross_dressed_df[['id','character_id','is_male', 'tokens']]
cross_dressed_df = cross_dressed_sentences[['id','character_id','is_male', 'sentence']]
column_of_interest = 'sentence'
...
test_data.to_csv('/projekte/tcl/users/keithan/projectcalderon/wp1-semantic-analysis/character_analysis/gender_analysis/gender_classifier/results/sentence_gender_predictions_probabilities.csv', index=False)
print(test_data['predictions'].value_counts())
